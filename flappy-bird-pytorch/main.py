# -*- coding: utf-8 -*-
"""训练和预测。
https://www.cnblogs.com/nebula-technology/p/16538205.html
"""
import random
import sys
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.layers = nn.ModuleDict({
            'c':
            nn.Sequential(nn.Linear(3, 12), nn.Sigmoid()),
            'a':
            nn.Linear(12, 2),
            'v':
            nn.Linear(12, 1),
        ...
        for key, value in self.layers.items():
            if rate >= 1.:
                ...
                for key in local.keys():
                    mix[key] = local.get(key) * (1 -
                                                 rate) + forign.get(key) * rate
...
def simulate(model: Model,
             ...
             epslion: float = .1,
             eval_step: int = None,
             env_args: dict = None) -> 'tuple[list,float,int]':
    """模拟游戏过程并收集数据。
    ...
    for _ in range(max_step):  # 看模型在不进行随机探索条件下能维持多少帧不摔机，这是评估标准
        ...
        if not env.playing:
            break
        livetime += 1
    return cache, sum(livetimes) / max(1,
                                       len(livetimes)) / batch_size, livetime
...
def train(
    ...
    for epoch in range(epochs):
        ...
        # 获取数据
        batch, accuracy, livetime = simulate(model=target_net,
        ...
        for state, action, next_state, reward in random.sample(
                cache, batch_size):
            states.append(state)
